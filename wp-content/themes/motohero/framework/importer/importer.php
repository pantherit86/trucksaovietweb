<?php
/**
 * motohero import demo data content
 *
 * @package motohero
 */
...
function inwave_importer()
{
    ...
    if (!current_user_can('manage_options')) {
        inwave_import_response('error', esc_html__('Error: Permission denied', 'motohero'));
    }
    ...
    if (!headers_sent()) {
        ...
        inwave_import_response('error', esc_html__('Error: Could not start session! Please try to turn off debug mode and error reporting', 'motohero'));
    }
    $importer = array();
    ...
    switch ($step) {
            ...
            foreach ($iw_post_types as $post_type) {
                switch ($post_type) {
                    ...
                    case 'woocommerce':
						...
						update_option('shop_thumbnail_image_size',array('width'=>300,'height'=>300,'crop'=>1));
        ...
        case 'finish':
            ...
            $wpdb->query($wpdb->prepare("update {$wpdb->postmeta} set meta_value = replace(meta_value,'http://inwavethemes.com/wordpress/motohero/wp-content',%s)", content_url()));

            $importer['base']->import_end();
            $importer['base']->step_done = $importer['base']->step_total;
            if (!$importer['base']->error_msg) {
                $message = '<b style="color:#444">'.esc_html__('Cheers! The demo data has been imported successfully! Please reload this page to finish!', 'motohero').'</b>';
            } else {
                $message = '<b style="color:#444">'.esc_html__('Data import completed!', 'motohero').'</b><br />' . '<div>' . $importer['base']->error_msg . '</div>';
            }
            $code = 'completed';
            break;
        default:
            inwave_import_response('error', sprintf(esc_html__('Error: step not found: %s', 'motohero'), $step));
            break;
    }
...
function inwave_get_import_data_types() {
    ...
    if (class_exists('portfolioBox')) {
        $data_types['post-type']['portfolio'] = esc_html__('Portfolios', 'motohero');
    }

	if (class_exists('Woocommerce')) {
        $data_types['post-type']['woocommerce'] =  esc_html__('Woocommerce', 'motohero');
    }
    if (defined('WPCF7_VERSION')) {
        $data_types['post-type']['wpcf7_contact_form'] = esc_html__('Contact Form 7', 'motohero');
		
    }

    $data_types['post-type']['media'] = esc_html__('Media', 'motohero');
    $data_types['post-type']['menu'] = esc_html__('Menus', 'motohero');
    $data_types['data-type']['widget'] = esc_html__('Widgets', 'motohero');

    if (class_exists('RevSlider')) {
        $data_types['data-type']['slider'] = esc_html__('Sliders', 'motohero');
    }
    return $data_types;
}
...
function inwave_import_portfolio(){
    ...
    // Import portfolio
    $iw_data = get_template_directory_uri() . '/framework/importer/data/portfolio.json';
...
function inwave_import_widgets(){
    ...
    $sidebars = array(
        ...
        'sidebar-services' => 'Sidebar Services',
        'sidebar-footer-widget-1' => 'Sidebar Footer widget 1',
        'sidebar-footer-widget-2' => 'Sidebar Footer widget 2',
        'sidebar-footer-widget-3' => 'Sidebar Footer widget 3'
    );
