...
class Inwave_Options_Machine {
		...
		foreach ($options as $value) {
			...
			//Start Heading
			 if ( $value['type'] != "heading" )
			 {
				...
				$output .= '<div id="section-'.esc_attr($value['id']).'" class="'.esc_attr($fold).'section section-'.esc_attr($value['type']).' '. $class .'">'."\n";
			...
			switch ( $value['type'] ) {
				...
				case 'select':
					...
					foreach ($value['options'] as $select_ID => $option) {
						...
						$theValue = $select_ID;
						$output .= '<option id="' . $select_ID . '" value="'.$theValue.'" ' . selected($inwave_theme_option[$value['id']], $theValue, false) . ' />'.$option.'</option>';
					 } 
					$output .= '</select></div>';
				break;
                ...
                case 'import_button':
					...
					$output .= '<h3>' . esc_html__('System Requirements', 'motohero') . '</h3>';
				...
				case 'slider':
					...
					$output .= '</ul>';
					$output .= '<a href="#" class="button slide_add_button">'.esc_html__('Add New Slide', 'motohero').'</a></div>';
				...
				case 'addoption':
					...
					$label = isset($value['option_label']) ? $value['option_label'] : esc_html__('Option', 'motohero');
					$add_btn_text = isset($value['add_btn_text']) ? $value['add_btn_text'] : esc_html__('Add New Option', 'motohero');
					$i = 0;
					if ($count > 1) {
						unset($addoptions[0]);
				...
				case 'social_link':
					...
					$addoptions = $inwave_theme_option[$value['id']];
					$count = count($addoptions);
					$label = isset($value['option_label']) ? $value['option_label'] : esc_html__('Social', 'motohero');
					$add_btn_text = isset($value['add_btn_text']) ? $value['add_btn_text'] : esc_html__('Add New Social', 'motohero');
					$i = 0;
					if ($count > 1) {
						unset($addoptions[0]);
						foreach ($addoptions as $addoption) {
				...
				case 'backup':
					...
					$output .= '<p><strong>'. esc_html__('Last Backup : ','motohero').'<span class="backup-log">'.$log.'</span></strong></p></div>'."\n";
					$output .= '<a href="#" id="of_backup_button" class="button">'.esc_html__('Backup Options','motohero').'</a>';
					$output .= '<a href="#" id="of_restore_button" class="button">'.esc_html__('Restore Options','motohero').'</a>';
					$output .= '</div>';
				...
				case 'transfer':
					...
					$output .= '<a href="#" id="of_import_button" class="button">'.esc_html__('Import Options','motohero').'</a>';
	...
	public static function optionsframework_media_uploader_function($id,$std,$mod){
		...
		if ( function_exists( 'wp_enqueue_media' ) ) {
			$uploader .= '<span class="button media_upload_button" id="'.esc_attr($id).'">'.esc_html__('Upload', 'motohero').'</span>';
			
			if(!empty($upload)) {$hide = '';} else { $hide = 'hide';}
			$uploader .= '<span class="button remove-image '. esc_attr($hide).'" id="reset_'. esc_attr($id) .'" title="' . esc_attr($id) . '">'.esc_html__('Remove', 'motohero'). '</span>';
		}
		else 
		{
			$output .= '<p class="upload-notice"><i>'.esc_html__('Upgrade your version of WordPress for full media support', 'motohero').'</i></p>';
		}
		...
		if(!empty($upload)){	
	    	...
	    	$uploader .= '<img style="background:grey;" class="of-option-image" id="image_'.esc_attr($id).'" src="'.esc_url($imageurl).'" alt="" />';
	    	$uploader .= '</a>';			
			}
		$uploader .= '</div>';
	...
	public static function optionsframework_slider_function($id,$std,$oldorder,$order){
		...
		if (!empty($val['title'])) {
			...
			$slider .= '<li><div class="slide_header"><strong>'.esc_html__('Slide', 'motohero').' '.$order.'</strong>';
		}
		...
		$slider .= '<a class="slide_edit_button" href="#">'.esc_html__('Edit','motohero').'</a></div>';
		
		$slider .= '<div class="slide_body">';
		
		$slider .= '<label>'.esc_html__('Title','motohero').'</label>';
		...
		$slider .= '<label>'.esc_html__('Image URL','motohero').'</label>';
		...
		$slider .= '<div class="upload_button_div"><span class="button media_upload_button" id="'.esc_attr($id.'_'.$order) .'">'.esc_html__('Upload','motohero').'</span>';
		
		if(!empty($val['url'])) {$hide = '';} else { $hide = 'hide';}
		$slider .= '<span class="button remove-image '. esc_attr($hide).'" id="reset_'. esc_attr($id .'_'.$order) .'" title="' . esc_attr($id . '_'.$order) .'">'.esc_html__('Remove','motohero').'</span>';
		$slider .='</div>' . "\n";
		...
		$slider .= '</div>';	
		$slider .= '<label>'.esc_html__('Link URL (optional)', 'motohero'). '</label>';
		...
		$slider .= '<label>'.esc_html__('Description (optional)', 'motohero').'</label>';
		$slider .= '<textarea class="slide of-input" name="'. $id .'['.$order.'][description]" id="'. $id .'_'.$order .'_slide_description" cols="8" rows="8">'.stripslashes($val['description']).'</textarea>';
	
		$slider .= '<a class="slide_delete_button" href="#">'.esc_html__('Delete','motohero').'</a>';
	...
	public static function optionsframework_addoption_function($id,$std,$label,$oldorder,$order){
		...
		$addoption .= '<a class="slide_edit_button" href="#">'.esc_html__('Edit','motohero').'</a></div>';
		...
		$addoption .= '<a class="slide_delete_button" href="#">'.esc_html__('Delete','motohero').'</a>';
	...
	public static function optionsframework_social_link_function($id,$std,$oldorder,$order){
		...
		$slider .= '<input class="slide of-input" name="'. esc_attr($id) .'['.$order.'][icon]" id="'. esc_attr($id .'_'.$order) .'_slide_link" value="'. $val['icon'] .'" />';

		$slider .= '<label>Link</label>';
		$slider .= '<input class="slide of-input" name="'. esc_attr($id) .'['.$order.'][link]" id="'. esc_attr($id .'_'.$order) .'_slide_link" value="'. $val['link'] .'" />';
	...
	static function getSystemRequireStatus() {
        ...
        <ul class="check-system-status">
            <li>
                <?php
                echo wp_kses(__('<span class="label1">PHP Version: </span>', 'motohero'), inwave_allow_tags('span'));
                if (version_compare($php_version,'5.3.0','<')) {
                    echo '<span class="invalid"><i class="fa fa-times"></i></span>' . sprintf(wp_kses(__('<span class="label2">Currently: %s</span>', 'motohero'), inwave_allow_tags('span')), $php_version) . '(' . esc_html__('Min: 5.3', 'motohero') . ')';
                } else {
                    echo '<span class="valid"><i class="fa fa-check"></i></span>' . sprintf(wp_kses(__('<span class="label2">Currently: %s</span>', 'motohero'), inwave_allow_tags('span')), $php_version);
                }
                ?>
            </li>
            <li>
                <?php
                echo wp_kses(__('<span class="label1">Maximum execution time: </span>', 'motohero'), inwave_allow_tags('span'));
                if ($max_execution_time < '90') {
                    echo '<span class="invalid"><i class="fa fa-times"></i></span>' . sprintf(wp_kses(__('<span class="label2">Currently: %s</span>', 'motohero'), inwave_allow_tags('span')), $max_execution_time) . '(' . esc_html__('Min: 90', 'motohero') . ')';
                } else {
                    echo '<span class="valid"><i class="fa fa-check"></i></span>' . sprintf(wp_kses(__('<span class="label2">Currently: %s</span>', 'motohero'), inwave_allow_tags('span')), $max_execution_time);
                }
                ?>
            </li>
            <li>
                <?php
                echo wp_kses(__('<span class="label1">Memory Limit: </span>', 'motohero'), inwave_allow_tags('span'));
                if (intval($memory_limit) < '128') {
                    echo '<span class="invalid"><i class="fa fa-times"></i></span>' . sprintf(wp_kses(__('<span class="label2">Currently: %s</span>', 'motohero'), inwave_allow_tags('span')), $memory_limit) . '(' . esc_html__('Min: 128M', 'motohero') . ')';
                } else {
                    echo '<span class="valid"><i class="fa fa-check"></i></span>' . sprintf(wp_kses(__('<span class="label2">Currently: %s</span>', 'motohero'), inwave_allow_tags('span')), $memory_limit);
                }
                ?>
            </li>
            <li>
                <?php
                echo wp_kses(__('<span class="label1">Uploads folder writable: </span>', 'motohero'), inwave_allow_tags('span'));
            ...
            <li>
                <?php
                echo wp_kses(__('<span class="label1">Start Session: </span>', 'motohero'), inwave_allow_tags('span'));

                if (!session_id()) {
                    session_start();
                }
                if(!session_id()){
                    echo '<span class="invalid"><i class="fa fa-times"></i></span>' . wp_kses(__('<span class="label2">Failed to start session. Please check session save_path in your php setting</span>', 'motohero'), inwave_allow_tags('span'));
                }else {
                    echo '<span class="valid"><i class="fa fa-check"></i></span>';
                }
                ?>
