<section class="classes" id="classes">
    ...
    if ($show_filter_bar) {
        ...
        <div class="categories">
            <div class="filters button-group" id="filters">
                <?php
                if (!empty($categories)) {
                    echo '<button class="filter is-checked" data-filter="*">' . __('All work', 'inwavethemes') . '</button>';
                    ...
                    <div class="classes-content" id="filtering-demo">
                        <section class="isotope portfolio-relate" id="our-class-main" style="">
                            ...
                            while ($query->have_posts()) :
                                ...
                                ?>
                                <div data-category="<?php echo $data_cat ? $data_cat : ''; ?>" class="<?php echo $class ? implode(' ', $class) : ''; ?> mix element-item col-sm-<?php echo 12 / (($number_column - 1) > 0 ? $number_column - 1 : 1); ?> col-md-<?php echo 12 / $number_column; ?> post_item duongca">
                                    <div class="item-info">
                                        <div class="image">
                                            <?php
                                                echo '<img src="' . $post_thumb[0] . '" alt=""/>';
                                            ?>
                                            <div class="control-overlay">
                                                <div class="port-effect">
                                                    <div class="port_title bg-bevelled"><a href="<?php echo get_permalink(); ?>"><span><?php echo get_the_title(); ?></span><span class="theme-bg iw-av-overlay"></span></a></div>
                                                    <?php if(!empty($cats)) :?>
                                                        <div class="port-category"><?php echo implode(' / ', $cats); ?></div>
                                                    <?php endif; ?>
                                                    <div class="port-detail"><a href="<?php echo get_permalink(); ?>"><?php _e('view detail', 'inwavethemes'); ?></a></div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
    ...
    <div class="load-product">
        ...
        if ($rs['success']) {
            echo '<button class="load-more load-portfolio theme-bg" id="load-more-class"><span class="ajax-loading-icon" style="margin-right: 10px; display: none;"><i class="fa fa-spinner fa-spin fa-2x"></i></span>' . __('Load More', 'inwavethemes') . '</button>';
            echo $rs['data'];
        } else {
            if (defined('DOING_AJAX') && DOING_AJAX) {
                echo '<button class="load-more load-portfolio all-loaded" id="load-more-class"><span class="ajax-loading-icon" style="margin-right: 10px; display: none;"><i class="fa fa-spinner fa-spin fa-2x"></i></span>' . __('All loaded', 'inwavethemes') . '</button>';
            }
        }
        wp_reset_postdata();
        ?>
