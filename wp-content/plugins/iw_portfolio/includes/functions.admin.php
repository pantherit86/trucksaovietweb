...
// Register Custom Post Type
    function iw_portfolio_add_post_types_portfolio() {
        ...
        $labels = array(
            'name' => _x('Trucks', 'Post Type General Name', 'inwavethemes'),
            'singular_name' => _x('Truck', 'Post Type Singular Name', 'inwavethemes'),
            'menu_name' => __('Trucks', 'inwavethemes'),
            'parent_item_colon' => __('Parent Item:', 'inwavethemes'),
            'all_items' => __('All Trucks', 'inwavethemes'),
...
if (!function_exists('iw_portfolio_add_taxonomy_class')) {
    ...
    function iw_portfolio_add_taxonomy_class() {
        ...
        $singular = 'Truck Category';
        $plural = 'Truck Categories';
        $utility = new iwcUtility();
...
function class_metabox_edit($tag) {
    ...
    <tr class="form-field">
        ...
        <td>
            ...
            <input style="width: 14%;" type='button' class='button' name='portfolio_cat_add_button' id='portfolio_cat_add_button' value='<?php echo __('Browse','inwavethemes'); ?>' />
            <br/>
        ...
        if (isset($_POST) && !empty($_POST)) {
            if (!get_option('iw_portfolio_settings')) {
                if (add_option('iw_portfolio_settings', $_POST)) {
                    $_SESSION['bt_message'] = $utility->getMessage(__('Setting has been saved successfully','inwavethemes'));
                } else {
                    $_SESSION['bt_message'] = $utility->getMessage(__('Error'), 'error');
                }
            } else {
                if (update_option('iw_portfolio_settings', $_POST)) {
                    $_SESSION['bt_message'] = $utility->getMessage(__('Setting has been updated successfully','inwavethemes'));
                } else {
                    $_SESSION['bt_message'] = $utility->getMessage(__('No change has been made','inwavethemes'));
                }
            }
            ...
            $_SESSION['bt_message'] = $utility->getMessage(__('No data send','inwavethemes'), 'error');
        }
    ...
    function saveIwPortfolioExtraField() {
        ...
        if (isset($_POST)) {
            ...
            if ($_POST['field_type'] != '') {
                ...
                $msg .= __('- Please select a field type<br/>','inwavethemes');
            }
...
//
            ...
            if ($msg == '') {
                ...
                if ($_POST['id']) {
                    ...
                    $_SESSION['bt_message'] = $utility->getMessage(__('Extrafield has been updated','inwavethemes'));
                    unset($_SESSION['extrafield']);
                    wp_redirect(admin_url('edit.php?post_type=iw_portfolio&page=iwp-add-extra-field&id=' . $_POST['id']));
                    return;
                } else {
                    ...
                    if ($ins) {
                        ...
                        $_SESSION['bt_message'] = $utility->getMessage(__('Extrafield has been created','inwavethemes'));
                        unset($_SESSION['extrafield']);
...
if (!function_exists('deleteExtraFields')) {
    ...
    function deleteExtraFields() {
        ...
        if (isset($_POST['fields']) && !empty($_POST['fields'])) {
            ...
            $_SESSION['bt_message'] = $utility->getMessage(__('Delete row(s) success','inwavethemes'));
        } else {
...
if (!function_exists('deleteExtraField')) {
    ...
    function deleteExtraField() {
        ...
        if (isset($_GET) && $_GET != '') {
            ...
            $wpdb->delete($wpdb->prefix . 'iw_portfolio_extrafields_category', array('extrafields_id' => $id));
            $wpdb->delete($wpdb->prefix . 'iw_portfolio_extrafields', array('id' => $id));
            $_SESSION['bt_message'] = $utility->getMessage(__('Deleted the row successfully','inwavethemes'));
        } else {
            $_SESSION['bt_message'] = $utility->getMessage(__('Please select a row to delete','inwavethemes'), 'error');
        }
        wp_redirect(admin_url('edit.php?post_type=iw_portfolio&page=extra-field'));
    }
